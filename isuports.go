...
// テナントテーブル操作用に接続する（DBを物理的に分ける可能性ある）
...
func Run() {
	...
	tenantDB, err = connectTenantDB()
	if err != nil {
		e.Logger.Fatalf("failed to connect tenant db: %v", err)
		return
	}
	tenantDB.SetMaxOpenConns(50)
	tenantDB.SetMaxIdleConns(25)
...
func playersAddHandler(c echo.Context) error {
	...
	for _, displayName := range displayNames {
		...
		p, err := retrievePlayer(ctx, tenantDB, id, v.tenantID)
		if err != nil {
...
func competitionScoreHandler(c echo.Context) error {
	...
	for {
		...
		if _, err := retrievePlayer(ctx, tenantDB, playerID, v.tenantID); err != nil {
			// 存在しない参加者が含まれている
			if errors.Is(err, sql.ErrNoRows) {
				return echo.NewHTTPError(
					http.StatusBadRequest,
